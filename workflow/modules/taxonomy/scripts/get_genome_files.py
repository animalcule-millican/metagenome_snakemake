...
def download_files(ftp_path, ref_path, taxa):
    """
    ...
    if not os.path.exists(os.path.join(ref_path, 'genome')):
        os.makedirs(os.path.join(ref_path, 'genome'))
    success = False
            ...
            with ftplib.FTP('ftp.ncbi.nlm.nih.gov') as ncbi:
                ...
                for file in file_list:
                    if "_genomic.fna.gz" in file and "_cds_from_genomic.fna.gz" not in file:
                        file_path = os.path.join(ref_path, "genome", taxa, file)
                        print(file_path)
...
def main():
    ...
    with ThreadPoolExecutor(max_workers=8) as executor:
            futures = [executor.submit(download_files, value['ftp'], ref_path, taxa) for value in gen_dict.values()]
